...
export class ChatHistoryDB {
  ...
  static async getChatHistory(userId) {
    try {
      ...
      const transformedChats = (Array.isArray(chats) ? chats : []).map(chat => {
        ...
        return {
          ...
          images: Array.isArray(chat.chat_images)
            ? chat.chat_images.map(image => ({
                id: image.id,
                url: image.image_url,
  ...
  static async getChat(userId, chatId) {
    try {
      // Check if chatId is in the old format (starts with 'chat_')
      if (chatId && chatId.startsWith('chat_')) {
        console.log('Converting old chat ID format to new format');
        // For old format IDs, try to get from local storage first
        const localChat = this.getLocalChat(userId, chatId);
        if (localChat) {
          return localChat;
        }
        // If not found locally, return null instead of querying Supabase
        return null;
      }
      ...
      if (!chatData) {
        console.log('Chat data is undefined for ID:', chatId);
        return null;
      }

      const sortedMessages = Array.isArray(chatData.messages)
        ? [...chatData.messages].sort((a, b) => new Date(a.created_at) - new Date(b.created_at))
        : [];

      // Return the chat with properly formatted data
      return {
        id: chatData.id,
        ...
        isClosed: chatData.is_closed,
        hasImage: chatData.has_image,
        imageUrl: chatData.image_url,
        imageName: chatData.image_name,
        images: Array.isArray(chatData.chat_images) ? chatData.chat_images.map(image => ({
          id: image.id,
          url: image.image_url,
          name: image.image_name,
          fileSize: image.file_size,
